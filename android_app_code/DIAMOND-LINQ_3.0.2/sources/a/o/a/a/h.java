...
public class h implements Interpolator {
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f345a.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f345a[i2]) {
                length = i2;
        ...
        float[] fArr = this.f345a;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f346b[i];
        }
        float[] fArr2 = this.f346b;
        float f3 = fArr2[i];
    ...
    private void a(Path path) {
        ...
        if (min > 0) {
            this.f345a = new float[min];
            this.f346b = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, (float[]) null);
                this.f345a[i2] = fArr[0];
                this.f346b[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f345a[0])) <= 1.0E-5d && ((double) Math.abs(this.f346b[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f345a[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f346b[i3] - 1.0f)) <= 1.0E-5d) {
                    int i4 = 0;
                    float f = 0.0f;
                    while (i < min) {
            ...
            sb.append(this.f345a[0]);
            sb.append(",");
            sb.append(this.f346b[0]);
            sb.append(" end:");
            int i6 = min - 1;
            sb.append(this.f345a[i6]);
            sb.append(",");
            sb.append(this.f346b[i6]);
