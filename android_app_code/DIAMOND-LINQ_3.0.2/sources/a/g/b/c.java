package a.g.b;

import a.d.g;
import a.g.a.a.c;
import a.g.a.a.h;
import a.g.f.f;
import android.content.Context;
import android.content.res.Resources;
import android.graphics.Typeface;
import android.os.Build;
import android.os.CancellationSignal;
import android.os.Handler;

public class c {

    /* renamed from: a  reason: collision with root package name */
    private static final j f138a;

    /* renamed from: b  reason: collision with root package name */
    private static final g<String, Typeface> f139b = new g<>(16);

    static {
        int i = Build.VERSION.SDK_INT;
        if (i >= 28) {
            f138a = new g();
        } else if (i >= 26) {
            f138a = new f();
        } else if (i >= 24 && e.a()) {
            f138a = new e();
        } else if (Build.VERSION.SDK_INT >= 21) {
            f138a = new d();
        } else {
            f138a = new j();
        }
    }

    public static Typeface a(Resources resources, int i, int i2) {
        return f139b.b(b(resources, i, i2));
    }

    private static String b(Resources resources, int i, int i2) {
        return resources.getResourcePackageName(i) + "-" + i + "-" + i2;
    }

    public static Typeface a(Context context, c.a aVar, Resources resources, int i, int i2, h.a aVar2, Handler handler, boolean z) {
        Typeface typeface;
        if (aVar instanceof c.d) {
            c.d dVar = (c.d) aVar;
            boolean z2 = false;
            if (!z ? aVar2 == null : dVar.a() == 0) {
                z2 = true;
            }
            typeface = f.a(context, dVar.b(), aVar2, handler, z2, z ? dVar.c() : -1, i2);
        } else {
            typeface = f138a.a(context, (c.b) aVar, resources, i2);
            if (aVar2 != null) {
                if (typeface != null) {
                    aVar2.a(typeface, handler);
                } else {
                    aVar2.a(-3, handler);
                }
            }
        }
        if (typeface != null) {
            f139b.a(b(resources, i, i2), typeface);
        }
        return typeface;
    }

    private static Typeface b(Context context, Typeface typeface, int i) {
        c.b a2 = f138a.a(typeface);
        if (a2 == null) {
            return null;
        }
        return f138a.a(context, a2, context.getResources(), i);
    }

    public static Typeface a(Context context, Resources resources, int i, String str, int i2) {
        Typeface a2 = f138a.a(context, resources, i, str, i2);
        if (a2 != null) {
            f139b.a(b(resources, i, i2), a2);
        }
        return a2;
    }

    public static Typeface a(Context context, CancellationSignal cancellationSignal, f.b[] bVarArr, int i) {
        return f138a.a(context, cancellationSignal, bVarArr, i);
    }

    public static Typeface a(Context context, Typeface typeface, int i) {
        Typeface b2;
        if (context == null) {
            throw new IllegalArgumentException("Context cannot be null");
        } else if (Build.VERSION.SDK_INT >= 21 || (b2 = b(context, typeface, i)) == null) {
            return Typeface.create(typeface, i);
        } else {
            return b2;
        }
    }
}
